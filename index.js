...
if(usePairingCode && !uno.authState.creds.registered) {
    ...
    // Anti Call
    uno.ev.on('call', async (fatihh) => {
    let botNumber = await uno.decodeJid(uno.user.id)
    ...
    let pa7rick = await uno.sendTextWithMentions(tihh.from, `*${uno.user.name}* tidak bisa menerima panggilan ${tihh.isVideo ? `video` : `suara`}. Maaf @${tihh.from.split('@')[0]} kamu akan diblockir. Jika tidak sengaja silahkan hubungi Owner untuk dibuka !`)
    uno.sendContact(tihh.from, global.owner, pa7rick)
    await sleep(8000)
    await uno.updateBlockStatus(tihh.from, "block")
    }
    }
    }
    })
    ...
    uno.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    uno.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Welcome To ${metadata.subject} @${num.split("@")[0]}` })
                } else if (anu.action == 'remove') {
                    uno.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split("@")[0]} Leaving To ${metadata.subject}` })
